...
@section('content')
    @php
        ...
        if (is_array($oldItems)) {
            $initialItems = filled($oldItems) ? $oldItems : [[]];
        } else {
            $initialItems = $order->orderItems
                ->map(fn ($item) => ['product_id' => $item->product_id, 'jumlah_pcs' => $item->jumlah_pcs])
                ->all();
            $initialItems = filled($initialItems) ? $initialItems : [[]];
        }
    @endphp
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="alamat_kirim" id="alamat_kirim" rows="4" class="form-control @error('alamat_kirim') is-invalid @enderror">{{ old('alamat_kirim', $order->alamat_kirim) }}</textarea>
                            @error('alamat_kirim')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
